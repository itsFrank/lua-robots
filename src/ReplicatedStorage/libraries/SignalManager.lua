...
function SignalManager:new(name, eventType, dontCreateNew)
	if RunService:IsServer() then
		if eventType ~= SignalManager.Enum.SignalType.RemoteFunction and eventType ~= SignalManager.Enum.SignalType.RemoteEvent then
...
function SignalManager.__newindex(table, key, value)
	...
	local signal
	if table._eventType == SignalManager.Enum.SignalType.RemoteFunction then
		signal = Instance.new("RemoteFunction", table._signalFolder)
		signal.OnServerInvoke = value
	elseif table._eventType == SignalManager.Enum.SignalType.RemoteEvent then
